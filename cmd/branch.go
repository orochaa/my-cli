...
var branchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		branchesData := utils.ExecSilent("git", "branch", "-a")
		...
		if isRemoteBranch(selectedBranch) {
			...
			utils.Exec("git", "checkout", "-b", remoteBranch)
			utils.Exec("git", "pull", remoteOrigin, remoteBranch)
		} else {
			originsData, err := utils.Exec("git", "remote")
			if err != nil {
				return
			}
			...
			utils.Exec("git", "pull", origin, branch)
		}
	},
}
